<script lang="ts">
	...
	function drawThumbnail(node: HTMLCanvasElement, bitmap: ImageBitmap | null | undefined) {
		const paint = (value: ImageBitmap | null | undefined) => {
			...
			node.getContext('2d')?.drawImage(value, 0, 0);
		};
		paint(bitmap);
		return { update: paint };
	}
	...
	function cardStyle(f: number): string {
		...
		const offset = delta === 0 ? 0 : side * (CARD_W * (1 + (Math.abs(delta) - 1) * (1 - OVERLAP)));
		const w = CARD_W * scale;
		return `left:calc(50% + ${offset + dragOffset}px - ${w / 2}px);width:${w}px;height:${100 * scale}%;opacity:${scale};z-index:${10 - Math.abs(delta)};pointer-events:${delta === 0 ? 'auto' : 'none'};transition:${dragStartX === null ? 'left 120ms ease, opacity 120ms ease, width 120ms ease, height 120ms ease' : 'none'};`;
	}
</script>
...
<div
	...
	onwheel={wheelMove}
	style:cursor={dragStartX === null ? 'grab' : 'grabbing'}
>
	{#each windowFrames as f (f)}
		<div class="fc-card" class:fc-center={f === centerFrame} style={cardStyle(f)}>
			{#if f === centerFrame}
				...
				></video>
			{:else if thumbs[f]}
				<canvas class="fc-thumb" use:drawThumbnail={thumbs[f]} aria-label="{APP_CONSTANTS.strings.frameLabel} {f}" />
			{:else}
