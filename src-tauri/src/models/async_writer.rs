        ...
        tokio::spawn(async move {
            while let Some(task) = rx.recv().await {
                match task {
                    ...
                    WriteTask::UpdatePipe {
                        pipe_id,
                        new_content,
                        reply,
                    } => {
                        let result =
                            Self::update_pipe_in_composer(&path, &pipe_id, &new_content, &format)
                                .await;
                        let _ = reply.send(result);
                    }
                }
    ...
    async fn append_to_composer(
        ...
        if let Some(pipes) = composer.get_mut("pipes").and_then(|p| p.as_array_mut()) {
            let pipe: serde_json::Value =
                serde_json::from_str(pipe_json).map_err(|e| e.to_string())?;
            pipes.push(pipe);
        }

        let new_content = serde_json::to_string_pretty(&composer).map_err(|e| e.to_string())?;
    ...
    async fn update_pipe_in_composer(
        ...
        if let Some(pipes) = composer.get_mut("pipes").and_then(|p| p.as_array_mut()) {
            if let Some(pipe) = pipes
                .iter_mut()
                .find(|p| p.get("id").and_then(|id| id.as_str()) == Some(pipe_id))
            {
                *pipe = serde_json::from_str(new_content).map_err(|e| e.to_string())?;
            }
        }
