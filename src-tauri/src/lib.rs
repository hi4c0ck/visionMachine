...
fn init_database(app_data_dir: &PathBuf) -> Result<PathBuf, String> {
    ...
    if db_path.exists() {
        log::info!("[DB] Database already exists at: {:?}", db_path);
        return Ok(db_path);
    }
    ...
    log::info!("[DB] Created new database at: {:?}", db_path);
    Ok(db_path)
}
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Err(e) = std::fs::create_dir_all(&logs_dir) {
                log::warn!("[Setup] Could not create logs dir: {}", e);
            } else {
                log::info!("[Setup] Logs directory created: {:?}", logs_dir);
            }
